...
my(@Options, $verbose, $seqid, $start, $end, $width, $revcom, $bam_fn, $fa_fn, 
             $consfreq, $unmask, $list, $nesoni, $ansi);
setOptions();
...
$consensus = REVERSE . $consensus . RESET if $ansi;
$consensus = " " . $consensus . "\n";
print $consensus;
...
#  my $read = uc $a->query->dna;
  ...
  # print it out
#  my $strand = $a->strand < 0 ? '-' : '+';
  my $strand = $a->strand < 0 ? STRAND_NEG : STRAND_POS;
#  $strand = FAINT . $strand . RESET if $ansi;
  my $cigar = $a->cigar_str;
#  $cigar = FAINT . $cigar . RESET if $ansi;
  print $strand, $read, " ", $cigar, "\n";
  $row++;
...
sub tweaked_read {
  ...
  for my $p (@{$a->cigar_array}) {
    my($op, $count) = @$p;
    if ($op eq 'I') {
      push @ins, substr($read, $offset, $count);
      substr $read, $offset, $count+1, BASE_INSERT;
    }
    elsif ($op eq 'D') {
      substr $read, $offset, 0, (BASE_DELETE)x$count;
    }
    $offset += $count;
  }
...
sub setOptions {
  ...
  @Options = (
    ...
    {OPT=>"a|ansi!",     VAR=>\$ansi,  DEFAULT=>0, DESC=>"Use ANSI ESC codes for BOLD etc"},
  );
