...
#    {OPT=>"sep=s", VAR=>\$sep, DEFAULT=>':', DESC=>"Separator string for IDs"},
#    {OPT=>"l|minlen=i", VAR=>\$minlen, DEFAULT=>31, DESC=>"Minimum length"},
#    {OPT=>"s|shrimp=s", VAR=>\$shrimp, DEFAULT=>'', DESC=>"Shrimp hits file"},
  );
...
sub usage {
  print "Usage: $0 [options] split_scaffs.fna shrimp_hits.txt\n";
  foreach (@Options) {
    printf "  --%-13s %s%s.\n",$_->{OPT},$_->{DESC},
