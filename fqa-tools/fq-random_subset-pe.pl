...
my $Lfn = shift @ARGV or die "please specify left.fq file";
print STDERR "Left reads: $Lfn\n";
-r $Lfn or die "can't read '$Lfn'";
open my $Lfh, "zcat -f $Lfn |";

my $Rfn = shift @ARGV or die "please specify right.fq file";
print STDERR "Right reads: $Rfn\n";
-r $Rfn or die "can't read '$Rfn'";
open my $Rfh, "zcat -f $Rfn |";
...
sub usage {
  print "Usage: $0 [options] <left.fq[.gz]> <right.fq[.gz]>\n";
  foreach (@Options) {
    printf "  --%-13s %s%s.\n",$_->{OPT},$_->{DESC},
